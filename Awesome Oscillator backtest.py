...
# https://github.com/messfin/quant-trading/blob/master/MACD%20oscillator%20backtest.py
...
def awesome_signal_generation(df,method):
    ...
    # Get values as numpy arrays for faster access
    opens = signals['Open'].values
    closes = signals['Close'].values
    ao = signals['awesome oscillator'].values
    ma1 = signals['awesome ma1'].values
    ma2 = signals['awesome ma2'].values

    for i in range(2, len(signals)):
        # awesome oscillator saucer signals
        open_i, close_i = opens[i], closes[i]
        open_i1, close_i1 = opens[i-1], closes[i-1]
        open_i2, close_i2 = opens[i-2], closes[i-2]
        ao_i, ao_i1, ao_i2 = ao[i], ao[i-1], ao[i-2]
        
        sig = 0
        if (open_i > close_i and 
            open_i1 < close_i1 and 
            open_i2 < close_i2 and
            ao_i1 > ao_i2 and
            ao_i1 < 0 and 
            ao_i < 0):
            sig = 1
        elif (open_i < close_i and 
              open_i1 > close_i1 and 
              open_i2 > close_i2 and
              ao_i1 < ao_i2 and
              ao_i1 > 0 and
        ...
        ma1_i, ma2_i = ma1[i], ma2[i]
        if ma1_i > ma2_i:
            sig = 1
            if current_cumsum + sig > 1:
                sig = 0
        elif ma1_i < ma2_i:
            sig = -1
            if current_cumsum + sig < 0:
                sig = 0
